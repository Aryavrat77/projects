...
def read_distance_values(filepath, assignment_number):
...
def calculate_mean_squared_error(assignment_number, file_letter, search_type):
    ...
    if (assignment_number == 5):
        max_iterations = 3 # Change as needed
        result_file = f"PA{assignment_number}-{file_letter}-OurOutput-Iter={max_iterations}.txt"
    else:
    ...
    values1 = read_distance_values(filepath1, assignment_number)
    values2 = read_distance_values(filepath2, assignment_number)
...
for search_type in search_types:
    for debug_file_letter in debug_file_letters: 
        calculate_mean_squared_error(5, debug_file_letter, search_type)
