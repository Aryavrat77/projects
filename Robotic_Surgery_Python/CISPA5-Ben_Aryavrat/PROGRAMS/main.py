...
from PROGRAMS.generateOutputPA5 import generate_output_PA5
from PROGRAMS.readFiles.readModes import read_modes
...
def main(problem_number, debug_file_letter, isKnown=True, search_type="Linear"):
    ...
    max_iterations = 9 # mainly for PA5

    if (problem_number == 5):
        output_filename = f"PA{problem_number}-{debug_file_letter}-OurOutput-Iter={max_iterations}.txt"  # Update 'A' as needed for different test cases+
    else :
    ...
    if (problem_number == 5):
        modes = read_modes(f'PROGRAMS/data/Problem{problem_number}Modes.txt')
        tol = 1e-5
        print(f"Generating output for PA5 file {debug_file_letter}")
        generate_output_PA5(output_dir, output_filename, frames, Atip, vertices, triangles, modes, max_iterations, tol)
...
if __name__ == "__main__":
    ...
    for debug_file_letter, is_known in zip(debug_file_letters, known_status):
            main(5, debug_file_letter, is_known, "Linear") # change problem number as needed
